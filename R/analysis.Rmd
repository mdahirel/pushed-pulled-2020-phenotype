...
(the number *X* in the "expX_trait.csv" refers to the generation of the experiment. 1: directly taken from the expanding landscapes; 2: one generation of common garden; 3: two generations of common garden)
...
ppc_ribbon(yrep=(predict(mod_size,summary=FALSE)), ## the blue bands represent the posterior predictions, the black line the obs
...
data_mvt<-raw_mvt %>% 
                ...
                summarise(obs_time=sum(path_duration),
...
mod_activity <- brm(bf(mean_activity~context+(1|Mix/IDgroup/IDpop),
               nlf(phi~1/invphi),
               invphi~1),
...
mod_sinuosity <- brm(bf(scale(mean_sinuosity)~context+(1|Mix/IDgroup/IDpop)),
  ...
  save(list = c("mod_activity","mod_speed","mod_sinuosity"), file = here("R_output", "models_mvt.Rdata"))
  }
```
...
###prediction intervals around each point
ppc_ribbon(yrep=(predict(mod_activity,summary=FALSE)),
           x=rank(predict(mod_activity)[,1]),
           y=data_mvt$mean_activity,
           prob = 0.5, prob_outer=0.95)

ppc_ribbon(yrep=(predict(mod_speed,summary=FALSE)),
           x=rank(predict(mod_speed)[,1]),
           y=scale(data_mvt$mean_speed)[,1],
           prob = 0.5, prob_outer=0.95)

ppc_ribbon(yrep=(predict(mod_sinuosity,summary=FALSE)),
           x=rank(predict(mod_sinuosity)[,1]),
           y=scale(data_mvt$mean_sinuosity)[,1],
           prob = 0.5, prob_outer=0.95)
```

Looking at these quick-and-dirty summaries, it is fairly easy to see that all three models all perform reasonably well and all come to the same conclusion: no clear differences between contexts. So we only focus on the "activity" model, for simplicity, in the main text of the article.
...
- `Bloc`: the testing block. A legacy column from the original, pre-common garden, expansion landscapes. Can be ignored (especially given `stock` are their own block, and that would make it extremely problematic to include it)
...
- only keep the first and last generation of testing (in the denity-independent experiment, some intermediate tests were also carried on, but we don't use them)
...
- for the density-dependent experiment: convert `Density` to a centred dummy variable (see e.g. doi:10.1111/j.2041-210X.2010.00012.x). Among the many advantages, this means that the intercept is on the "average" density experienced; this include the random effects, for which it is certainly more appropriate:
...
mod_fec <- brm(bf(Fecundity~context+(1|Mix/IDgroup/IDpop),
         ...
         chains=4,iter=4000,
         prior=c(set_prior("normal(3.8,0.5)",class="Intercept"), ## see supplementary for rationale
                 set_prior("normal(0,1)",class="b"),
                 set_prior("normal(0,1)",class="sd"),
         ...
         seed=42,control=list(adapt_delta=0.999,max_treedepth=15),backend="cmdstanr"
)
...
mod_fec_dens <- brm(bf(Fecundity~context*Density_centred+(1|Mix/IDgroup/IDpop),
         ...
         prior=c(set_prior("normal(3.8,0.5)",class="Intercept"),
                 ...
                 set_prior("normal(0,0.2)",class="sd",dpar="zi",group="Mix"),
                set_prior("normal(0,1)",nlpar="invshape",lb=0)),
         seed=42,control=list(adapt_delta=0.999,max_treedepth=20),backend="cmdstanr"
)
...
ppc_ribbon(yrep=(predict(mod_fec,summary=FALSE)),
...
- `Neggs_start` and `Neggs_arrival`: number of successfully parasitised eggs in the starting patch and the arrival patch, respectively.
...
- we add a scaled version of that variable, who can possibly serve as predictor (see a bit later)
...
```{r summary-model-alt}
ppc_ribbon(yrep=(predict(mod_disp_alt,summary=FALSE)),
           x=rank(predict(mod_disp_alt)[,1]),
           y=data_disp$Neggs_arrival,
           prob = 0.5, prob_outer=0.95)

ppc_ribbon(yrep=(predict(mod_disp_dens_alt,summary=FALSE)),
           x=rank(predict(mod_disp_dens_alt)[,1]),
           y=data_disp_dens$Neggs_arrival,
           prob = 0.5, prob_outer=0.95)
```

The predictions looks mostly good-ish, but there are a bit of over and underpredictions compared to previous models, which may signal some overdispersion (especially in the model with density; try also other `pp_checks`)

How do we account for that? 

We have reasons to think that we need to control for the total number of eggs laid. After all, once you more or less filled the starting patch, you can only increase fecundity if you increase dispersal. And also, there may be a dispersal-fecundity syndrome. So let's do this:
...
```{r summary-model}
ppc_ribbon(yrep=(predict(mod_disp,summary=FALSE)),
