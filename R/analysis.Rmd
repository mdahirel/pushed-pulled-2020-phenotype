...
Looking at these quick-and-dirty summaries, it is fairly easy to see that all three models all perform reasonably well and all come to the same conclusion: no clear differences between contexts. So we only focus on the "activity" model, for simplicity, in the main text.
...
We'd probably need to use cross-validation instead given the number of "problematic" observations based on `pareto_k`, but model comparison suggest that adding `Neggs_total` as covariate improve the models. In any case, it is easy to see that our main interpretations are unchanged by the choice of model, for instance by changing the source model in the plots below. So let's go to plotting now.
