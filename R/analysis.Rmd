...
ggplot(preds_size) +
  stat_eye(aes(x=Location,y=.value),.width=c(0.01,0.95))+
...
p1 <- data_fec %>% 
  ...
  ggplot()+
  stat_eye(aes(Location, .value),.width=c(0.01,0.95),fill="#d95f02")+
...
p2 <- data_fec_dens %>%  
  ...
  mutate(Treatment = fct_relevel(Treatment,"reduced connectivity", after=Inf)) %>% 
  ggplot()+
  stat_eye(aes(Location, .value,fill=Density), .width=c(0.01,0.95), position="dodge")+
...
p3 <- data_fec_dens %>%  
  ...
  ggplot()+
  stat_eye(aes(Location, diff), .width=c(0.01,0.95))+
...
p3 <- posterior_samples(mod_disp_dens) %>% 
  select(contains("Density_centred")) %>% 
  mutate(.draw=1:dim(.)[1]) %>% 
  mutate(b_stock=0) %>% #dummy column to facilitate additions below
  rename(baseline=b_Density_centred) %>% 
  pivot_longer(cols=-c(baseline,.draw)) %>%  
  ...
  mutate(Treatment = fct_relevel(Treatment,"reduced connectivity", after=Inf)) %>% 
  ggplot()+
  stat_eye(aes(Location, value+baseline), .width=c(0.01,0.95))+
  geom_hline(yintercept=0,lty=2)+
  scale_x_discrete("Experimental context")+
