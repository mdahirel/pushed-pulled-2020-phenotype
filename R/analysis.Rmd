...
p3<-post_size %>% 
  group_by(.draw) %>% 
  mutate(stockmean=mean(subset(mean,Treatment=="stock"))) %>% 
  mutate(stocksd=mean(subset(sd,Treatment=="stock"))) %>% 
  ungroup() %>% 
  mutate(pnorm=pnorm(stockmean-stocksd,mean,sd)) %>% 
  ggplot()+
  stat_eye(aes(x=Location,y=pnorm),normalize="xy")+
  scale_x_discrete("Experimental context")+
  scale_y_continuous("Proportion of \"small\" wasps")+
    facet_grid(cols=vars(Treatment),scales="free_x",space="free_x")+
  geom_hline(yintercept = pnorm(-1,0,1),lty=2)+
  theme_bw()
...
p2<-ggplot(test) +
  stat_eye(aes(x=Location,y=high/low),fill="#7570b3",alpha=0.7)+
