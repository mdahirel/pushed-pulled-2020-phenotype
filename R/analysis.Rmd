...
mod_size=brm(bf(scale(tibia)~0+context+(1|Mix/IDgroup/IDpop)+(1|IDindiv)),
        data=data_size,
        chains=4,iter=4000,
...
ggplot(post_size) +
  stat_eye(aes(x=Location,y=mean),.width=c(0.01,0.95))+
...
mod_fec_dens=brm(bf(Fecundity|trials(90)~0+context+context:Density_centred+(1|Mix/IDgroup/IDpop)+(1|obsID),
            zi~0+context+context:Density_centred+(1|Mix/IDgroup/IDpop)),
         data=data_fec_dens,family=zero_inflated_binomial,
         chains=4,iter=4000,
         prior=c(set_prior("normal(0,1.5)",class="b"),
                 set_prior("normal(0,1)",class="b",
                           coef=c("contextreferencecore:Density_centred","contextreferenceedge:Density_centred",
                                  "contextreducedconnectednesscore:Density_centred","contextreducedconnectednessedge:Density_centred",
                                  "contextstock:Density_centred")),
                 set_prior("normal(0,1)",class="sd"),
                 set_prior("normal(0,1.5)",class="b",dpar="zi"),
...
mod_disp_dens=brm(bf(Neggs_arrival|trials(Neggs_all)~0+context+context:Density_centred+Neggs_all_scaled+(1|Mix/IDgroup/IDpop)),
...
data_disp %>% 
  ...
  ungroup() %>% 
  add_fitted_draws(mod_disp,re_formula=NA) %>% 
  group_by(Location, Treatment, .draw) %>% 
  summarise(.value=mean(.value/Neggs_all)) %>% 
  ungroup() %>% 
  ggplot()+
  stat_eye(aes(Location, .value))+
  geom_jitter(data=data_disp,aes(Location,Neggs_arrival/Neggs_all))+
  #stat_pointinterval(data=data_disp,aes(Location,Neggs_arrival/Neggs_all), col="red")+
  scale_x_discrete("Experimental context")+
...
data_disp_dens %>% 
  #group_by(Location, Treatment, Density) %>% 
  #mutate(Neggs_all_scaled = mean(Neggs_all_scaled)) %>% 
  #ungroup() %>% 
  add_fitted_draws(mod_disp_dens,re_formula=NA) %>% 
  ## apparent discrepancy between mean of points and mean posterior due to random effect
  ## add suppl mat with mean including random effects to show this?
  group_by(Location, Treatment, Density, .draw) %>% 
  summarise(.value=mean(.value/Neggs_all)) %>% 
  ungroup() %>% 
  ggplot()+
  stat_eye(aes(Location, .value,fill=Density),position="dodge")+
  geom_jitter(data=data_disp_dens,aes(Location,Neggs_arrival/Neggs_all, col=Density),position=position_dodge(width=0.5))+
  #stat_pointinterval(data=data_disp,aes(Location,Neggs_arrival/Neggs_all), col="red")+
  scale_x_discrete("Experimental context")+
  scale_y_continuous("Effective dispersal rate")+
  facet_grid(cols=vars(Treatment),scales="free_x",space="free_x")+
...
posterior_samples(mod_disp_dens) %>% 
  select(contains("b_")) %>% 
  select(contains("Density_centred")) %>% 
  mutate(.draw=1:dim(.)[1]) %>% 
  ...
  mutate(Location=case_when(str_detect(name,"stock")~"stock",
         ...
         Treatment=case_when(str_detect(name,"stock")~"stock",
                            ...
                            str_detect(name,"reduced")~"reduced connectivity")) %>% 
  ggplot()+
  stat_eye(aes(Location, value))+
  geom_hline(yintercept=0,lty=2)+
  scale_x_discrete("Experimental context")+
  scale_y_continuous("Net effect of density on dispersal (logit scale)")+
  facet_grid(cols=vars(Treatment),scales="free_x",space="free_x")+
  theme_bw()
