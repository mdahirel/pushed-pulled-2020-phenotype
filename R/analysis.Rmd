...
mod_popsize <- brm(bf(Peggs_est~logit(K-(K-M0)*exp(-r*time_since_founding))+obs,
                      ...
                      logitM0~0+Treatment+Treatment:scale(dist_to_front_prev)+(1|Mix)+(scale(dist_to_front_prev)|1|landscapeID)+(scale(dist_to_front_prev)|2|landscapeID:Patch),
                      log10r~0+Treatment+(1|Mix)+(1|1|landscapeID)+(1|2|landscapeID:Patch),obs~0+(1|Macro)+(1|spacetimeID),
                      nlf(phi~10^log10phi),
                      log10phi~1,
                      nl=TRUE),
